import 'package:flutter/material.dart';

class IPhone11ProX84 extends StatelessWidget {
  const IPhone11ProX84({Key key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8F8F8),
      body: SizedBox(
        width: 375.0,
        height: 812.0,
        child: Stack(
          children: <Widget>[
            SizedBox(
              width: 375.0,
              height: 812.0,
              child: Column(
                children: <Widget>[
                  // const IOSStatusBarBlack(),
                  Spacer(flex: 83),
                  Align(
                    alignment: Alignment(0.03, 0.0),
                    child: SizedBox(
                      width: 344.0,
                      height: 350.0,
                      child: Stack(
                        children: <Widget>[
                          Container(
                            // Group: Контент
                            alignment: Alignment.center,
                            width: 344.0,
                            height: 322.0,
                            child: SizedBox(
                              width: 344.0,
                              height: 322.0,
                              child: Column(
                                children: <Widget>[
                                  Align(
                                    alignment: Alignment(0.03, 0.0),
                                    child: SizedBox(
                                      width: 305.0,
                                      height: 36.0,
                                      child: Text(
                                        'Введите код доступа',
                                        style: TextStyle(
                                          fontFamily: 'SF Pro Display',
                                          fontSize: 30.0,
                                          color: const Color(0xFFFD4F6A),
                                          fontWeight: FontWeight.w700,
                                        ),
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                  ),
                                  Spacer(flex: 8),
                                  Align(
                                    alignment: Alignment(0.02, 0.0),
                                    child: SizedBox(
                                      width: 279.0,
                                      height: 38.0,
                                      child: Text.rich(
                                        TextSpan(
                                          style: TextStyle(
                                            fontFamily: 'SF Pro Display',
                                            fontSize: 16.0,
                                            color: const Color(0xFF222222),
                                          ),
                                          children: [
                                            TextSpan(
                                              text:
                                                  'Мы отправили вам смс с кодом потверждения на',
                                            ),
                                            TextSpan(
                                              text: ' ',
                                              style: TextStyle(
                                                color: Colors.black,
                                              ),
                                            ),
                                            TextSpan(
                                              text: '+7 (708) 371-28-80',
                                              style: TextStyle(
                                                color: const Color(0xFFFD4F6A),
                                                fontWeight: FontWeight.w500,
                                              ),
                                            ),
                                          ],
                                        ),
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                  ),
                                  Spacer(flex: 28),
                                  Container(
                                    // Group: Цифры
                                    alignment: Alignment.center,
                                    width: 344.0,
                                    height: 74.0,
                                    child: SizedBox(
                                      width: 344.0,
                                      height: 74.0,
                                      child: Stack(
                                        children: <Widget>[
                                          Container(
                                            // Group: 4
                                            alignment: Alignment.center,
                                            width: 315.0,
                                            height: 74.0,
                                            child: SizedBox(
                                              width: 315.0,
                                              height: 74.0,
                                              child: Row(
                                                children: <Widget>[
                                                  Container(
                                                    alignment:
                                                        Alignment(0.02, -0.02),
                                                    width: 71.0,
                                                    height: 74.0,
                                                    decoration: BoxDecoration(
                                                      borderRadius:
                                                          BorderRadius.circular(
                                                              4),
                                                      border: Border.all(
                                                        width: 2.0,
                                                        color: const Color(
                                                            0xFFFD4F6A),
                                                      ),
                                                    ),
                                                    child: SizedBox(
                                                      width: 18.0,
                                                      height: 33.0,
                                                      child: Text(
                                                        '4',
                                                        style: TextStyle(
                                                          fontFamily:
                                                              'SF Pro Display',
                                                          fontSize: 28.0,
                                                          color: const Color(
                                                              0xFF222222),
                                                          letterSpacing:
                                                              1.0000000305175782,
                                                          fontWeight:
                                                              FontWeight.w600,
                                                        ),
                                                        textAlign:
                                                            TextAlign.center,
                                                      ),
                                                    ),
                                                  ),
                                                  Spacer(flex: 47),
                                                  Align(
                                                    alignment:
                                                        Alignment(0.0, -0.02),
                                                    child: SizedBox(
                                                      width: 18.0,
                                                      height: 33.0,
                                                      child: Text(
                                                        '5',
                                                        style: TextStyle(
                                                          fontFamily:
                                                              'SF Pro Display',
                                                          fontSize: 28.0,
                                                          color: const Color(
                                                              0xFF222222),
                                                          letterSpacing:
                                                              1.0000000305175782,
                                                          fontWeight:
                                                              FontWeight.w600,
                                                        ),
                                                        textAlign:
                                                            TextAlign.center,
                                                      ),
                                                    ),
                                                  ),
                                                  Spacer(flex: 74),
                                                  Align(
                                                    alignment:
                                                        Alignment(0.0, -0.02),
                                                    child: SizedBox(
                                                      width: 17.0,
                                                      height: 33.0,
                                                      child: Text(
                                                        '2',
                                                        style: TextStyle(
                                                          fontFamily:
                                                              'SF Pro Display',
                                                          fontSize: 28.0,
                                                          color: const Color(
                                                              0xFF222222),
                                                          letterSpacing:
                                                              1.0000000305175782,
                                                          fontWeight:
                                                              FontWeight.w600,
                                                        ),
                                                        textAlign:
                                                            TextAlign.center,
                                                      ),
                                                    ),
                                                  ),
                                                  Spacer(flex: 70),
                                                  Align(
                                                    alignment:
                                                        Alignment(0.0, -0.02),
                                                    child: SizedBox(
                                                      width: 18.0,
                                                      height: 33.0,
                                                      child: Text(
                                                        '9',
                                                        style: TextStyle(
                                                          fontFamily:
                                                              'SF Pro Display',
                                                          fontSize: 28.0,
                                                          color: const Color(
                                                              0xFF222222),
                                                          letterSpacing:
                                                              1.0000000305175782,
                                                          fontWeight:
                                                              FontWeight.w600,
                                                        ),
                                                        textAlign:
                                                            TextAlign.center,
                                                      ),
                                                    ),
                                                  ),
                                                ],
                                              ),
                                            ),
                                          ),
                                          Positioned(
                                            left: 91.0,
                                            child: Container(
                                              // Group: 7
                                              alignment: Alignment.center,
                                              width: 71.0,
                                              height: 74.0,
                                              child: Container(
                                                width: 71.0,
                                                height: 74.0,
                                                decoration: BoxDecoration(
                                                  borderRadius:
                                                      BorderRadius.circular(
                                                          4.0),
                                                  border: Border.all(
                                                    width: 2.0,
                                                    color:
                                                        const Color(0xFFFD4F6A),
                                                  ),
                                                ),
                                              ),
                                            ),
                                          ),
                                          Positioned(
                                            right: 91.0,
                                            child: Container(
                                              // Group: 5
                                              alignment: Alignment.center,
                                              width: 71.0,
                                              height: 74.0,
                                              child: Container(
                                                width: 71.0,
                                                height: 74.0,
                                                decoration: BoxDecoration(
                                                  borderRadius:
                                                      BorderRadius.circular(
                                                          4.0),
                                                  border: Border.all(
                                                    width: 2.0,
                                                    color:
                                                        const Color(0xFFFD4F6A),
                                                  ),
                                                ),
                                              ),
                                            ),
                                          ),
                                          Positioned(
                                            right: 0,
                                            child: Container(
                                              // Group: 2
                                              alignment: Alignment.center,
                                              width: 71.0,
                                              height: 74.0,
                                              child: Container(
                                                width: 71.0,
                                                height: 74.0,
                                                decoration: BoxDecoration(
                                                  borderRadius:
                                                      BorderRadius.circular(
                                                          4.0),
                                                  border: Border.all(
                                                    width: 2.0,
                                                    color:
                                                        const Color(0xFFFD4F6A),
                                                  ),
                                                ),
                                              ),
                                            ),
                                          ),
                                        ],
                                      ),
                                    ),
                                  ),
                                  Spacer(flex: 32),
                                  Align(
                                    alignment: Alignment(0.01, 0.0),
                                    child: SizedBox(
                                      width: 255.0,
                                      height: 14.0,
                                      child: Text(
                                        'Запросить код повторно',
                                        style: TextStyle(
                                          fontFamily: 'SF Pro Display',
                                          fontSize: 12.0,
                                          color: const Color(0xFFFD4F6A),
                                          letterSpacing: 1.000000030517578,
                                          fontWeight: FontWeight.w500,
                                        ),
                                        textAlign: TextAlign.center,
                                      ),
                                    ),
                                  ),
                                  Spacer(flex: 41),
                                  Container(
                                    // Group: Кнопка
                                    alignment: Alignment.center,
                                    width: 344.0,
                                    height: 51.0,
                                    child: Container(
                                      alignment: Alignment.center,
                                      width: 344.0,
                                      height: 51.0,
                                      decoration: BoxDecoration(
                                        borderRadius:
                                            BorderRadius.circular(4.0),
                                        color: const Color(0xFFFD4F6A),
                                      ),
                                      child: SizedBox(
                                        width: 95.0,
                                        height: 37.0,
                                        child: Text(
                                          'Продолжить',
                                          style: TextStyle(
                                            fontFamily: 'SF Pro Display',
                                            fontSize: 16.0,
                                            color: Colors.white,
                                            fontWeight: FontWeight.w600,
                                            height: 2.31,
                                          ),
                                          textAlign: TextAlign.center,
                                        ),
                                      ),
                                    ),
                                  ),
                                ],
                              ),
                            ),
                          ),
                          Positioned(
                            bottom: 0,
                            child: Container(
                              // Group: Group 1013
                              alignment: Alignment.center,
                              width: 344.0,
                              height: 143.0,
                              child: Container(
                                width: 344.0,
                                height: 143.0,
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(4.0),
                                  color: Colors.white,
                                ),
                                child: SizedBox(
                                  width: 344.0,
                                  height: 131.0,
                                  child: Stack(
                                    children: <Widget>[
                                      Container(
                                        // Group: Mask Group
                                        alignment: Alignment.center,
                                        width: 344.0,
                                        height: 131.0,
                                        child: Container(
                                          // [Rectangle 129] Group masks aren't supported.
                                          width: 344.0,
                                          height: 131.0,
                                          color: Colors.red,
                                        ),
                                      ),
                                      Positioned(
                                        left: 17.0,
                                        bottom: 4.0,
                                        child: Container(
                                          // Group: Провал
                                          alignment: Alignment.center,
                                          width: 309.0,
                                          height: 111.0,
                                          child: Container(
                                            // Group: Group 1012
                                            alignment: Alignment.center,
                                            width: 309.0,
                                            height: 111.0,
                                            child: SizedBox(
                                              width: 309.0,
                                              height: 111.0,
                                              child: Stack(
                                                alignment: Alignment.topCenter,
                                                children: <Widget>[
                                                  Positioned(
                                                    top: 23.0,
                                                    child: Container(
                                                      // Group: Контент
                                                      alignment:
                                                          Alignment.center,
                                                      width: 127.0,
                                                      height: 55.0,
                                                      child: SizedBox(
                                                        width: 127.0,
                                                        height: 55.0,
                                                        child: Text(
                                                          'Провал :(',
                                                          style: TextStyle(
                                                            fontFamily:
                                                                'SF Pro Display',
                                                            fontSize: 24.0,
                                                            color: const Color(
                                                                0xFFCF3C3C),
                                                            fontWeight:
                                                                FontWeight.w700,
                                                            height:
                                                                2.2916666666666665,
                                                          ),
                                                          textAlign:
                                                              TextAlign.center,
                                                        ),
                                                      ),
                                                    ),
                                                  ),
                                                  Positioned(
                                                    bottom: 0,
                                                    child: SizedBox(
                                                      width: 309.0,
                                                      height: 34.0,
                                                      child: Text.rich(
                                                        TextSpan(
                                                          style: TextStyle(
                                                            fontFamily:
                                                                'SF Pro Display',
                                                            fontSize: 14.0,
                                                            color: const Color(
                                                                0xFF222222),
                                                          ),
                                                          children: [
                                                            TextSpan(
                                                              text:
                                                                  'Такого кода не существует. Может вы ошиблись? ',
                                                            ),
                                                            TextSpan(
                                                              text:
                                                                  'Попробуйте еще раз',
                                                              style: TextStyle(
                                                                fontWeight:
                                                                    FontWeight
                                                                        .w500,
                                                              ),
                                                            ),
                                                          ],
                                                        ),
                                                        textAlign:
                                                            TextAlign.center,
                                                      ),
                                                    ),
                                                  ),
                                                  // Positioned(
                                                  //   left: 142.0,
                                                  //   child: const Close(),
                                                  // ),
                                                ],
                                              ),
                                            ),
                                          ),
                                        ),
                                      ),
                                    ],
                                  ),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                  Spacer(flex: 44),
                  // const IOSKeyboardLightNumeric(),
                ],
              ),
            ),
            Container(
              width: 375.0,
              height: 812.0,
              decoration: BoxDecoration(
                color: Colors.black.withOpacity(0.15),
              ),
            ),
          ],
        ),
      ),
    );
  }
}
